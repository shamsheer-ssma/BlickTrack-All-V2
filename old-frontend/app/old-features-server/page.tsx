...
async function getFeaturesData() {
  try {
    ...
    const categoriesResult = await client.query(`
      SELECT id, "displayName", description, "isActive", "createdAt", "updatedAt"
    ...
    const featuresResult = await client.query(`
      SELECT id, "displayName", description, "categoryId", "isActive", "createdAt", "updatedAt"
      FROM features 
      ORDER BY "displayName"
    `);
    ...
    console.log('Features loaded:', featuresResult.rows.length);
...
export default async function OldFeaturesServerPage() {
  const { categories, features } = await getFeaturesData();
          ...
          <div className="flex justify-between items-center py-6">
            <div>
              ...
              <p className="text-sm text-gray-500 mt-1">
                Loaded {categories.length} categories and {features.length} features
              </p>
            </div>
